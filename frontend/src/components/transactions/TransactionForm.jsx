...
/**
 * Normalise an `initial` transaction coming from the backend:
 * - date may be a full ISO string ("2026-09-23T00:00:00.000Z") → slice to YYYY-MM-DD
 * - _id field is irrelevant to the form but preserved for the parent
 */
function normaliseInitial(t) {
  if (!t) return blank;
  return {
    ...blank,
    ...t,
    date: t.date ? new Date(t.date).toISOString().slice(0, 10) : blank.date,
  };
}

export default function TransactionForm({ initial, onSubmit, onCancel }) {
  const [f,   setF]   = useState(() => normaliseInitial(initial));
  ...
  function change(e) {
    ...
    setF((x) => ({
      ...x,
      [name]: value,
      // Reset category when switching type so an invalid category isn't submitted
      ...(name === 'type' ? { category: '' } : {}),
    }));
  }
  ...
  return (
    <form className="form-grid" onSubmit={submit}>
      ...
      <label>
        Date
        <input name="date" type="date" value={f.date} onChange={change} />
      </label>
      ...
      <label>
        Payment Method
        <select name="paymentMethod" value={f.paymentMethod} onChange={change}>
