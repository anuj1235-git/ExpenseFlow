...
/** Return a consistent string id for a transaction (handles _id and id fields). */
const txId = (t) => (t?._id ?? t?.id)?.toString();
...
export default function Transactions() {
  ...
  const [view,         setView]         = useState(null);  // transaction for detail modal
  const [edit,         setEdit]         = useState(null);  // transaction for edit modal
  const [del,          setDel]          = useState(null);  // id string for delete modal
  ...
  const reset    = () => { setFilters(DEFAULT_FILTERS); setShowAdvanced(false); };
  ...
  return (
    <>
      ...
      <section className="card filters-card">
        ...
        {showAdvanced && (
          <div className="advanced-filters">
            ...
            <label className="adv-label">
              Min Amount
              <input type="number" min="0" value={filters.amtMin}
                onChange={(e) => set('amtMin', e.target.value)} placeholder="0" />
            </label>
            <label className="adv-label">
              Max Amount
              <input type="number" min="0" value={filters.amtMax}
                onChange={(e) => set('amtMax', e.target.value)} placeholder="Any" />
            </label>
          </div>
        )}
      </section>
      ...
      <TransactionDetailModal
        ...
        onDelete={(id) => { setDel(id); setView(null); }}
      />
      ...
      <Modal open={!!edit} title="Edit Transaction" onClose={() => setEdit(null)}>
        ...
        />
        {editLoading && <p style={{ textAlign: 'center', color: '#64748b', marginTop: 8 }}>Saving…</p>}
      </Modal>
      ...
      <Modal
        ...
        actions={
          <>
            <button className="btn secondary" onClick={() => setDel(null)} disabled={delLoading}>
              Cancel
            </button>
            <button className="btn danger-btn" onClick={handleDelete} disabled={delLoading}>
